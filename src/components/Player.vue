...
<template>
    ...
    <div id="player" @mousemove="move_pos" @mouseup="end_move">
        <div id="frame">
            ...
            <p id="song-name" @click="list=true">{{ song_name }}</p>
        ...
        >
            <ul id="playlist">
                <li v-if="playlist.length == 0"><el-empty></el-empty></li>
                <li v-for="song in playlist" class="playlist-item">
...
export default {
    ...
    methods:{
        ...
        add_to_playlist(id){
            if(this.playlist.filter(item => item.id == id).length > 0){
                return
            }
            axios.get('http://127.0.0.1:3000/song/detail?ids='+id)
            .then(res=>{
        ...
        play_playlist(flag=false){
            this.load_music(this.playlist[this.play_index].id)
            if(this.playlist.length == 1){
                if(flag){this.play_index = 1}
                else{
                    this.play_index = 0
                }
            }else{
                this.play_index ++;
                if(this.play_index >= this.playlist.length){
                    this.play_index = 0;
                }
            }
        }
    ...
    mounted(){
        window.electron.on_play_music(this.add_to_playlist)
